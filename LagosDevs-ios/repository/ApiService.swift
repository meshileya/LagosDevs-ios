...
class ApiService{
    ...
    func fetchUserInfoAsync(userInfoUrl: String, handler: @escaping CompletionUserInfoHandler){
        Alamofire.request(userInfoUrl, method: .get, parameters: nil, headers : nil).responseJSON
        {
            response in
            switch response.result {
            case .success(_):
                ...
                do {
                    let decoder = JSONDecoder()
                    let mainResponse = try decoder.decode(UserInfoDto.self, from: data)
                    handler(true, mainResponse, nil)
                } catch let error {
                    print(error)
